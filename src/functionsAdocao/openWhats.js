...
export default async function openWhats(nomePet, imagemPet) {
    ...
    const imagem = imagemPet; // A URL da imagem do pet
    const token = 'f396e16e46b034eb322bf418b073080d71e2957f'; // Token de acesso do Bitly
    const api = 'https://api-ssl.bitly.com/v4/shorten';

    try {
        // Envia a requisição para encurtar o link
        ...
        const fullMensagem = `${mensagem} ${shortenedLink}`; // Mensagem completa

        const url = `whatsapp://send?phone=${numero}&text=${encodeURIComponent(fullMensagem)}`;

        // Verifica se o WhatsApp está instalado e tenta abrir o link
